...
# 3.	Создайте метод __init__(), внутри которого будут определены два приватных
# атрибута: 1) _index - передается параметром и 2) _state - принимает первое значение из словаря states
...
# два атрибута: 1) name - передается параметром, является публичным
# и 2) _plant - принимает объект класса TomatoBush, является приватным
...
bush = TomatoBush(int(input('Введите количество томатов на кусте: ')))
gardener = Gardener(input('Введите имя садовника: ').capitalize(), bush)
